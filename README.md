...
|---|---|---|
| **v3.0.9** | Reply-tone hard rules + work-pool boundary at session endpoints. When you write in Chinese (or any non-English language), AI replies stay in your language without weaving English mid-sentence — English appears only for proper nouns, established terms with no clean translation, or as parenthetical traceability tags; ground truth identifiers (file paths, git SHAs, version numbers, commit hashes, code identifiers) stay allowed inline; governance internal IDs (`§` codes / `D-XXX` / `F-XXX` / `priority #X` / `Track A/B/C` / `Phase #N` / `Reflection Follow-Through Protocol` / invented `Capital_Snake_Case` names) are banned from sentence subjects. Choice options now lead with impact / outcome / what it means for your goal — not "which file changed / how it's done / what tech". Two ghost-reference holes patched: header startup inventory was missing `§0a` Layer Separation; bootstrap backup list was missing 4 v3.0.7+ governance files (`dev/wizards/playbook.md`, `dev/wizards/README.md`, `dev/templates/spec_template.md`, `dev/templates/runbook_template.md`) — re-installing on an existing project would skip backing them up. SESSION_LOG drift killed at the source: handoff prompt is now codified as "seed context only" (its prior-session done / changed / pending is HISTORY); this session's `Done` / `Changed` / `Files-edited` fields strictly reflect work performed in THIS conversation; cross-session continued tasks record only the increment, not cumulative completion. | Replies feel like a colleague writing in your language, not a half-translated paragraph with English fragments stuck in. Choice prompts make the trade-off obvious in the first half-line — you don't read implementation detail to figure out what each option means for your work. Re-installing on a project that already has wizards / templates no longer skips backing them up. Most importantly: SESSION_LOG no longer drifts every session — old INIT.md users reported repeatedly fixing prior-session done items being copy-pasted into the current session record; that drift is now hard-stopped by governance, not relying on AI mental tracking. |
...
Snapshot status (as of 2026-05-07 — v3.0.9):
- AGENTS/INIT rule parity: verified (350-check automated regression — 261 main + 89 legacy auto-chain)
- AGENTS.md governance scope: 530 → 687 lines (+29.6%) at v3.0.5 Tier 2 integration; v3.0.6 visual cue refresh + hint simplification line-count-neutral; cumulative −6.4% vs v2.x baseline (734); all rules + 325 grep-anchors preserved (212 baseline + R29×12 + R30×6 + entry-cap×3 + reply-behavior×6 + R31×17 + R32×34 + R33×35)
