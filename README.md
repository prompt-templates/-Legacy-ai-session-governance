...
|---|---|---|
| **v3.0.10** | Worktree session startup now respects the skip-worktree convention — when you open Claude Code inside a `git worktree`, governance correctly resolves `dev/SESSION_HANDOFF.md` / `dev/SESSION_LOG.md` from the main repo path instead of failing with file-not-found or silently creating empty fallback files in the worktree. The fallback is detected via `git rev-parse --git-common-dir` (parent dir is the main repo); the "create a minimal version" rule now only fires when both worktree and main repo genuinely lack the files. Release verification harness gets the same treatment: `bash docs/qa/run_checks.sh` is documented to run from the main repo path — worktree-path execution triggers spurious `H01` (`.legacy_last_run` cross-tree drift) and `R27-10` (skip-worktree files absent in worktree) failures that are by-design per skip-worktree convention, not real failures. Also includes a small zh-TW residual sweep: `入面` / `令` / `用戶` tokens at the v3.0.8 row converged to formal-written register matching the v3.0.9 sweep policy. | If you use git worktrees for parallel features or per-session isolation, governance now matches reality — worktree session startup just works without manual file copying or empty fallback creation. Release verification gives consistent results regardless of execution path, and the canonical execution locus is documented in `§3c` instead of leaving operators to discover skip-worktree behavior via spurious harness failures. zh-TW v3.0.8 row reads as formal written register end-to-end. |
