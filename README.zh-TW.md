...
| 版本 | 變更內容 | 對你的意義 |
|---|---|---|
| **v3.0.9** | 回覆語氣 hard rules + 工作池邊界喺 session 兩端定型。中文（或其他非英文）情景下 AI 回覆全程同你語言一致，不再逐句中英混雜——英文僅限專有名詞、無乾淨翻譯嘅既有術語，或括號 trace 用途；ground truth 標識（file path / git SHA / 版本號 / commit hash / 代碼識別字）容許 inline；governance 內部 ID（`§` 編號 / `D-XXX` / `F-XXX` / `priority #X` / `Track A/B/C` / `Phase #N` / `Reflection Follow-Through Protocol` / 自創 `Capital_Snake_Case` 名）禁止作句子主語。選擇題嘅選項以影響 / 後果 / 對你目標嘅意義開頭，不再以「改邊個 file / 點做 / 用乜技術」嘅機制角度開頭。修補兩個 ghost reference 漏：header startup inventory 漏咗 `§0a` Layer Separation；bootstrap backup list 漏咗 4 個 v3.0.7+ governance file（`dev/wizards/playbook.md` / `dev/wizards/README.md` / `dev/templates/spec_template.md` / `dev/templates/runbook_template.md`）——重 install 既存項目時，呢 4 個 file 會冇備份保護。SESSION_LOG drift 由源頭 hard-stop：handoff prompt 正式 codified 為「seed context only」（佢入面講嘅上 session done / changed / pending 屬 HISTORY）；本 session 嘅 `Done` / `Changed` / `Files-edited` 嚴格只 reflect 本 conversation 真實做過嘅 work；跨 session 持續 task 只 record increment，不重複 cumulative completion。 | 回覆似返一個用你語言寫嘢嘅同事，不再係夾住英文碎片嘅半翻譯段落。選擇題嘅 trade-off 喺第一行半已經清楚——你毋須讀 implementation detail 先理解每個選項對你工作嘅意義。重 install 已 install 過 wizards / templates 嘅項目，呢 4 個 file 不再漏備份。最重要：SESSION_LOG 唔再每 session drift——舊版 INIT.md 用戶反映過反覆要修「上 session done 條目俾人抄入本 session record」嘅 drift 痛點，而家由 governance hard-stop，不再靠 AI mental tracking。 |
