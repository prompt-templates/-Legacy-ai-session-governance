...
| 版本 | 变更内容 | 对你的意义 |
|---|---|---|
| **v3.0.9** | AI 回复会跟你写的语言：当你切换中文（或其他非英文语言）时，AI 全程用你的语言回复，不会逐句中英混杂（英文仅用于专有名词、无干净翻译的既有术语、或括号 trace）。当 AI 给你选选项时，每个选项的标题以「对你工作的意义」开头——例如「任何 clone main 的用户即时拿到新行为」，而非「commit + push main + cut tag」的机制角度。重 install 已有 wizards / templates 的项目时，这 4 个 file（`dev/wizards/playbook.md`、`dev/wizards/README.md`、`dev/templates/spec_template.md`、`dev/templates/runbook_template.md`）不再漏备份。最重要：每个 session 的 log 不再 carry forward 上 session 的「done」事项——closeout 只记录本 conversation 真实做过的 work；跨 session 持续的 task 只记本次 increment，不重复 cumulative completion。 | 回复像一个用你语言写东西的同事，不再是夹着英文碎片的半翻译段落。选择题的 trade-off 在第一行半就已经清楚——你无需读 implementation detail 才理解每个选项对你工作的意义。重 install 已 install 过 wizards / templates 的项目，这 4 个 file 不再漏备份。最重要：SESSION_LOG 不再每 session drift——旧版 INIT.md 用户反映过反复要修「上 session done 条目被抄进本 session record」的 drift 痛点，现在由 governance hard-stop，不再靠 AI mental tracking。 |
