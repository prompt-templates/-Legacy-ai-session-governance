...
| **v3.0**（v3.0.1 / v3.0.2 patches を含む） | ガバナンス文書を大幅にスリム化：AGENTS.md は 734 行から 504 行へ（−31.3%）、ルールはすべて保持。セッション起動時のシステムプロンプト・トークンコストが約 15.6% 減少。Legacy quarantine 機構で 89 件の履歴ドリフト防止チェックを自動チェーンの第二層ハーネスに分離 — メインチェックは軽量化、ただしリリース時の legacy バイパスは禁止のため履歴セーフガードが無音で失われることはない。v3.0.1 はリリース後の文書同期ガバナンスを追加（R29 系列チェック）、README / index.html のドリフトを自動検出。v3.0.2 はリリース／マージゲートを 4 段階ライフサイクル（リリース前検証 / リリース実行 / リリース後クリーンアップ / 観察期）に拡張、R30 系列 enforcement 付き。`dev/SESSION_STATE_DETAIL.md` や `dev/PROJECT_MASTER_SPEC.md` を作成済みのユーザーが再インストールしても、これらが自動でバックアップされる（データ安全なアップグレードパス）。 | システムプロンプト中のガバナンス文が減る → ルール遵守率が向上（業界データ：短いルール約 89% vs 冗長な記述約 35%）；リリース後関連ファイルのドリフトを自動検出（README、リリースノート、公開ページの stat counter が同期）；ローカルファイルはアップグレード時に保持；複数 LLM 間で相互運用性あり（Claude Code、Claude Cowork、OpenAI Codex CLI、Gemini CLI、Web LLMs）— フック依存ゼロ。 |
| **v2.8** | INIT-only 配布境界を強化。`INIT.md` と README から内部メンテナンスツール参照を削除し、INIT が未同梱ファイルを参照したら失敗する回帰チェックを追加。 | `INIT.md` だけで導入するユーザー環境での初回インストール失敗を防止し、今後の境界逸脱も自動検知できる。 |
| **v2.7** | 引き継ぎ・ログ肥大化対策を全面更新し、30ケースの増加シナリオで検証。引き継ぎ出力は安定して簡潔になり、履歴が増えた場合は古い内容が自動で起動の主経路から外れる。 | 起動が速くなり、コンテキスト浪費を抑えつつ、重要な引き継ぎ情報は維持。ストレス条件では起動ペイロードを最大 **16,096 tokens** 削減し、全シナリオで必須引き継ぎ項目を保持。 |
| **v2.6** | `SESSION_LOG.md` から引き継ぎメモを探す際、以前は「ファイル内で最後に出てくるもの」を使っていたが、ログを整理・アーカイブした後は物理的に最後の位置にある古いものが選ばれることがあった。今は「最新の日付のエントリの中にあるもの」を使う仕様に変更し、どんな順序で並んでいても正しく見つかるようになった。`INIT.md` の10ステップ安全確認手順は、ファイル内に2箇所書かれていて8か所以上で文言がずれ始めていたため、先頭を「下の唯一の定義を参照」する3行のポインターに変更した。セッション開始・終了時に表示されるASCIIアート図の選択ルールも見直し、「前回と同じものを避ける」という跨セッションでは守れないルールを「同じセッション内で開始と終了の図を違うものにする」に変更した。ガバナンス文書を編集するだけの作業が「外部APIを呼ぶコード」と誤認されて `CODEBASE_CONTEXT.md` の事前作成を要求されることがあったため、純粋な文書編集はその条件を発動しないと明確化した。自動チェックは169項から210項に増え、Session IDの書式・禁止コマンドの網羅性・ファイル名のルール・closeout区切り線の規則など、これまでカバーできていなかった項目が加わった。 | ログを並べ替えたり整理した後でも、引き継ぎメモが正しく読み込まれる。ルールの重複や文言のずれが減り、AI動作の予測可能性がさらに向上する。 |
| **v2.5** | コアワークフロールールをAIが最優先で参照する位置へ移動（見落としやすい末尾から先頭付近へ）；冗長セクションを統合（差引-3行）；3つのワークフローギャップを修正 — テスト失敗時にAIがサイレントリトライせず報告、deviation stop後に再開フェーズを明示、曖昧な表現でセッション終了を誤認しない | コアルールのAI遵守率が向上；メンテナンス負荷軽減；障害・引き継ぎ時のAI動作がより予測可能 |
| **v2.4** | AIがPLAN段階でタスクリスクを評価 — 高リスクタスク（3ファイル以上、スコープ不明、破壊的操作、外部システム）はユーザー確認まで一時停止；セッションログを簡潔な記録形式に変更（エントリあたり約60%縮小）；アーカイブ閾値を800行から400行に引き下げ、AI起動時の無関係な履歴読み込みを削減 | タスクの誤解をコード変更前にキャッチ；AI起動の高速化；少ないスペースでより多くの履歴を保持 |
| **v2.3** | 体系的監査による7つの改善：AIがタスク実行前に理解内容を明示（PLAN確認ステップ）、ユーザー指示とガバナンスルールの衝突時に明示、実行中に前提が間違っていた場合は停止して報告、簡単な質問に4部構成の回答を強制しなくなった | タスク誤解の減少、オーバーライドの追跡可能化、誤った前提による無駄な作業を大幅削減 |
...
2. **セッション開始**：`AGENTS.md に従ってこのセッションを開始してください` と入力。AI が前回の続きを把握します。
3. **作業**：機能追加・バグ修正・文書作成 — 何でも AI に頼めます。
4. **終了**：`このセッションを収束し、完全な引き継ぎまで実行してください。` と入力。AI が **NEXT SESSION OPENING MESSAGE** メモを渡してくれます。
5. **次回セッション**：そのメモを最初のメッセージとして貼り付ける — するとステップ 2 に戻ります。

> **ステップ 5 で貼り付けを忘れたら？** 同じパソコン + 同じ AI ツール：AI が `dev/SESSION_LOG.md` に保存されたメモを自動で読み取り、続きを始めます。別のパソコン、またはウェブの AI（ChatGPT / claude.ai など）：AI はゼロから始まります — 貼り付けが必須です。貼り付けたほうが確実です。迷ったら貼る。
...
> **`Follow AGENTS.md` という短い指示ではなく、なぜ手動で貼り付けるのか？** ガバナンス設計自体は self-contained — AI は前回の handoff を `SESSION_LOG.md` から自動読み込みするはずです。しかし実テストでは、短い `Follow AGENTS.md` 指示は完全な起動シーケンスを約 70–85% の確率でしか発火しません（AI ツール・モデル・プラットフォームによる）。OPENING MESSAGE ブロックは明示的指示 — 冒頭 2 行で AI に 4 つのガバナンスファイルを順序通り読むよう指示し、ツール間の信頼性を約 95% 以上まで引き上げます（Codex / Claude Code / CoWork / Gemini CLI / ウェブ AI）。一度貼り付けるだけで不確定性が消えます。迷ったら貼り付けてください。
...
2026-05-01（v3.0.5）時点の要約：
- AGENTS/INIT 規則整合：検証済み（315 項自動回帰 — 226 メイン + 89 legacy 自動チェーン）
- AGENTS.md ガバナンス範囲：530 → 687 行（+29.6%）v3.0.5 Tier 2 統合のため；累計 −6.4% 対 v2.x baseline (734)；すべてのルールと 290 個の grep アンカーを完全保持（212 baseline + R29×12 + R30×6 + entry-cap×3 + reply-behavior×6 + R31×17 + R32×34 v3.0.5 Tier 2 統合）
