...
| **応答行動ガバナンス** | AI が偽装した自由質問で決定をユーザーに押し戻す／選択肢に劣悪な候補を埋める／過剰な確認質問を出す／未確認の事実を確定として提示／内部 `§` コードを文の主語として使う — §11a（v3.0.3）が 5 つの応答ルールを必須化：judgement-first、≤3 選択肢 + 推薦、≤3 仮説 + ≤3 質問、`UNVERIFIED` と `NA` の区別、平易な日本語の表層テキスト |
...
2026-05-01（v3.0.4）時点の要約：
- AGENTS/INIT 規則整合：検証済み（281 項自動回帰 — 192 メイン + 89 legacy 自動チェーン）
- AGENTS.md L4 削減：734 → 530 行（−27.8%）、すべてのルールと 256 個の grep アンカーを完全保持（212 baseline + R29×12 release-doc sync + R30×6 release-lifecycle 4-phase + entry-cap×3 + reply-behavior×6 + R31×17 closeout-clarity / startup-transparency / lifecycle-svg）
