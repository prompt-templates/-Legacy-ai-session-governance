...
```text
<前回出力の「NEXT SESSION OPENING MESSAGE」ブロックを原文のまま貼り付けてください。
 ツール変更・PC 変更・ウェブ AI への移行では必須。同じ PC + 同じツールなら
 AI 側の SESSION_LOG 自動フォールバックでも続けられますが、貼り付けたほうが確実です。>
```
