...
|---|---|---|
| **v3.0.9** | 返信トーン hard rules + ワークプール境界をセッション両端で定型化。ユーザーが中国語（または他の非英語言語）で書く場合、AI 返信はその言語で完結し英語を文中に織り込まない — 英語は固有名詞、きれいに翻訳できない既存用語、または括弧トレースのみ；ground truth 識別子（file path / git SHA / バージョン番号 / commit hash / コード識別子）はインラインで許可；governance 内部 ID（`§` コード / `D-XXX` / `F-XXX` / `priority #X` / `Track A/B/C` / `Phase #N` / `Reflection Follow-Through Protocol` / 創作的 `Capital_Snake_Case` 名）は文の主語として禁止。選択肢のラベルは影響 / 結果 / あなたの目標への意味から始まり、「どの file を変えるか / どうやるか / どの tech を使うか」のメカニズム視点ではなくなりました。2 つの ghost reference の穴を修復：header startup inventory に `§0a` Layer Separation が欠落；bootstrap backup list に 4 つの v3.0.7+ governance file（`dev/wizards/playbook.md` / `dev/wizards/README.md` / `dev/templates/spec_template.md` / `dev/templates/runbook_template.md`）が欠落 — 既存プロジェクトでの再 install 時、これら 4 つの file がバックアップから漏れていました。SESSION_LOG drift をソースから hard-stop：handoff prompt は「seed context only」として正式 codified（その内に書かれた前 session の done / changed / pending は HISTORY）；本 session の `Done` / `Changed` / `Files-edited` は厳密に本 conversation で実行した work のみを反映；session 跨ぎの継続タスクは increment のみを記録、cumulative completion を重複させません。 | 返信があなたの言語で書く同僚のようになり、英語の断片が混ざった半翻訳の段落ではなくなりました。選択肢の trade-off が最初の半行で明確 — 各選択肢が作業にとって何を意味するかを理解するために implementation detail を読む必要がありません。wizards / templates が既にある project への再 install で、これら 4 つの file がバックアップから漏れることがなくなりました。最も重要：SESSION_LOG が毎 session drift しない — 旧版 INIT.md ユーザーは「前 session の done 項目が今 session record にコピーされる」drift 痛点を繰り返し修正してきました；今は governance による hard-stop で、AI mental tracking に頼らなくなりました。 |
...
2026-05-07（v3.0.9）時点の要約：
- AGENTS/INIT 規則整合：検証済み（350 項自動回帰 — 261 メイン + 89 legacy 自動チェーン）
- AGENTS.md ガバナンス範囲：530 → 687 行（+29.6%）v3.0.5 Tier 2 統合のため；v3.0.6 のビジュアル更新と表現簡略化は行数中性；累計 −6.4% 対 v2.x baseline (734)；すべてのルールと 325 個の grep アンカーを完全保持（212 baseline + R29×12 + R30×6 + entry-cap×3 + reply-behavior×6 + R31×17 + R32×34 + R33×35）
