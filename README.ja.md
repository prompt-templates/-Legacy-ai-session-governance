...
|---|---|---|
| **v3.0.10** | Worktree セッション起動が skip-worktree 慣例に正しく対応：`git worktree` 内で Claude Code を開いた際、ガバナンスは `dev/SESSION_HANDOFF.md` と `dev/SESSION_LOG.md` をメイン repo パスから読み込むようになり、file-not-found エラーやワークツリーへの空ファイル作成を回避します。ワークツリー検出は `git rev-parse --git-common-dir`（その親ディレクトリがメイン repo パス）で行い、従来の「最小バージョンを作成」規則はワークツリーとメイン repo の両方にファイルが存在しない場合にのみ適用されます。リリース検証 harness も同様に明文化：`bash docs/qa/run_checks.sh` はメイン repo パスから実行する必要があります — ワークツリーパスから実行すると、spurious な `H01`（`.legacy_last_run` クロスツリードリフト）と `R27-10`（ワークツリー内に skip-worktree ファイルが存在しない）失敗が発生しますが、これらは skip-worktree 慣例による by-design であり、真の失敗ではありません。同時に小さな zh-TW 残留掃除：README.zh-TW.md v3.0.8 行の `入面` / `令` / `用戶` トークンを正式書面語に収斂し、v3.0.9 sweep ポリシーに整合。 | git ワークツリーで並列機能開発やセッションごとの作業領域分離を行う場合、ガバナンスルールが実態と一致するようになりました — ワークツリーセッションの起動が、手動ファイルコピーや空ファイル作成なしで動作します。リリース検証結果は実行パスに関係なく一貫しており、harness ドキュメントは skip-worktree 慣例を明示するため、利用者が spurious failures を通じて自己発見するコストがなくなります。README.zh-TW.md v3.0.8 行は全文を通じて正式書面語に収斂されました。 |
