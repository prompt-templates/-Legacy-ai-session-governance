...
|---|---|---|
| **v3.0.3** | (1) **§4 entry-size cap** — SESSION_LOG の各エントリを ≤110 行に厳格制限（verbatim handoff ブロックを含む）；release-class の詳細は新しい逃げ場 `dev/SESSION_STATE_DETAIL.md` に移動。(2) **§11a Reply Behavior** — AI の各応答に 5 つの必須ルール：judgement-first（自由質問を偽装してユーザーに決定を押し戻さない）、choice format（≤3 選択肢 + 検証可能な根拠付き推薦）、ambiguity handling（≤3 仮説 + ラウンドあたり ≤3 質問）、fact verification（`UNVERIFIED` と `NA` を区別）、平易な日本語の表層テキスト（内部 `§` コードを文の主語にしない）。9 件の新規回帰チェック（#167-175）が AGENTS / INIT のミラー整合と新マーカー行 MANDATORY REPLY DISCIPLINE を強制。 | 長期プロジェクトのアクティブな SESSION_LOG が常にコンパクト（100行超のリリース系エントリが起動読み込みを圧迫しない）。AI 応答がより明確で読みやすい — 「どう思う？」ループの減少、未確認の事実状態の明示、ガバナンス知識なしでも表層テキストが理解可能。両ルールは INIT.md 経由ですべてのインストールに反映される。 |
...
2026-05-01（v3.0.3）時点の要約：
- AGENTS/INIT 規則整合：検証済み（264 項自動回帰 — 175 メイン + 89 legacy 自動チェーン）
- AGENTS.md L4 削減：734 → 521 行（−29.0%）、すべてのルールと 227 個の grep アンカーを完全保持（212 baseline + R29×12 release-doc sync + R30×6 release-lifecycle 4-phase + entry-cap×3 + reply-behavior×6）
