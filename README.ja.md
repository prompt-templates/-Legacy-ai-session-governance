...
例：Codexはコード、Claudeは文書、Geminiは基盤調査を担当 — または研究プロジェクト、ライティング草稿、データ分析ワークブック、agent デザイン仕様の異なる部分を異なる AI ツールが処理。  
...
違います。AIがどう読み・変更し・検証し・引き継ぐかを決めるもので、コードの書き方は関係ありません。AI が支援するあらゆるプロジェクトに適用：リサーチ、ライティング、データ分析、agent デザイン、コーディングいずれも対応 — §3.6 wizard の profile selector が `research` / `writing` / `agent-design` / `data-analysis` / `coding` を明示的にサポート。
