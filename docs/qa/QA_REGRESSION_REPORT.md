...
- Pre-existing checks adjusted: EXPECTED_INDEX_COUNTER 366→374 (Task 1 framework +8 R33-58..65); LATEST_STABLE_TAG v3.0.10 (unchanged — post-tag polish only)
- Task 1 Language layer separation framework (R33-58..65, 2026-05-12 second batch): mirror parity AGENTS↔INIT — non-developer default (R33-58/59, `Assume the user may not be a software developer` sentence); `Language layer separation` framework name (R33-60/61); Layer 3 `Governance internal schema files` carve-out (R33-62/63); multilingual universality `applies to any natural language` (R33-64/65). Reframe-preserve verified: R33-28/29 (`do not weave English mid-sentence`) + R33-30/31 (`Ground truth identifiers allowed inline`) 既有 4 條 anchor key phrase 喺 reframe 後仍 hit。
