...
Date: 2026-05-12 (UTC, Task 3 + Landing audit + Matrix-QC + INIT dry-run + design-layer fixes + landing audit governance + closeout fix)
Scope: **2026-05-12 Root-fix Option 4 — Author-then-Violate root cause treatment:** matrix-qc 13-dim audit before v3.1.0-rc.1 surfaced 2 release blockers tracing to one root cause — same session authored §4 stray-file scan + §4 budget caps but its own closeout violated both (Current Baseline 7/6, Open Priorities 7/5, 3 strays deferred to Open Priority instead of executing (a)/(b)/(c) disposition). Three structural gaps: (1) §4 rules lacked mandatory visible output requirement (vs §3 PERSIST DOC_SYNC Matrix Scan); (2) §4 disposition silently allowed undocumented fourth path (defer to Open Priorities); (3) no same-session rule self-audit mechanism. §8b promotion threshold met. Root-fix Option 4 = combined three fixes: (a) §4 `Closeout stray-file disposition (mandatory visible output)` block + forbid Open Priority as disposition + named anti-pattern *Stray-as-Open-Priority*; (b) §4 `Closeout compactness budget check (mandatory visible output)` block; (c) §4 `Same-session rule self-audit (mandatory)` block + named anti-pattern *Author-then-Violate* promoted to permanent governance. AGENTS↔INIT mirror byte-aligned. DOC_SYNC "Governance rule change" row enhanced with closeout-relevant cross-reference. Harness R33-103..108 (+6 checks, 3 anchor pairs); EXPECTED_INDEX_COUNTER 411 → 417. **Prior 2026-05-12 Combined Option C + Closeout fix:** user-raised gaps surfaced — landing page substantive content audit missing on governance change (only mechanical data-target sync); §4 closeout trigger wording strict keyword (English-only examples, no intent detection); §4 rule 3 / skeleton fence syntax ambiguous (causing handoff blocks outside code fence). 8-finding audit revised Option C — conditional sub-rule pattern (matches §10b), generic surface list (avoids anti-hardcoding), enhance existing DOC_SYNC row (avoids new-row duplication per §3b consolidation). Three fixes: (1) §3 PERSIST adds conditional "User-facing artefact audit" sub-rule; (2) §4 closeout trigger extended to "detect by intent, not strict keyword match" + multilingual examples; (3) §4 rule 3 + skeleton template wording changed to explicit "triple-backtick markdown code block with `text` language tag". AGENTS↔INIT mirror byte-aligned. DOC_SYNC "Governance rule change" row enhanced (not new row). Harness R33-99..102 (+4 checks); EXPECTED_INDEX_COUNTER 407 → 411. **Prior 2026-05-12 Recommended 3 design-layer fixes — closes INIT install flow Path A:** three dedicated audits + user selected default for all three: (1) FILE 1 Section-Aware Merge Protocol with `INSTALL_MERGE_OK` confirmation gate (REPLACE / ADD / KEEP three-category-list before write); (2) FILE 2/3 first-10-lines duplicate-guard search for `@AGENTS.md` / `@./AGENTS.md`; (3) §5a step 5 Install mode line (`first-install` / `upgrade` / `partial`) added to dry-run plan output. Harness R33-94..98 (+5 checks: §5a Install mode mirror, Section-Aware Merge + INSTALL_MERGE_OK, FILE 2/3 dup guard wording, Section-Aware Merge header). EXPECTED_INDEX_COUNTER 402 → 407. **Prior 2026-05-12 INIT install flow dry-run fix — 9 stuck points (critical 6 + important 3) eliminated:** comprehensive dry-run of INIT.md install + upgrade flow surfaced 12 stuck points; user selected Path B fixing 9 (critical + important), deferring 3 design-layer fixes for separate plan. Changes: install flow continuity grand statement at INIT.md head; immediate-continue wording after PROFILE.md template (user-reported stuck point); back-to-back Message 1+2 explicit; corrected Message 2 skip condition (skip Message 2 only, proceed to Message 3 not Quick Start); explicit wizard-complete transitions for Message 2 and Message 3 picks A; clarified Message 3 trigger after Message 2 outcome (handles skipped path). Harness R33-92/93 (+2 checks); EXPECTED_INDEX_COUNTER 400 → 402. **Prior 2026-05-12 Landing page audit + Matrix-QC upgrade (reflects Task 1 / Task 2 / Task 3):** Landing page added 2 new feature cards (f13 External Knowledge Surface + f14 Language Register Discipline), updated f6 description (Task 2 hardening), added scenario sc4 (Notes Live in Notion or Obsidian), added FAQ entry, in all 4 locales. Stat counter 396 → 400. Matrix-QC agent (gitignored) upgraded 10-dim → 13-dim with new Dim 11 (External KB integrity) + Dim 12 (Reply register discipline) + Dim 13 (Pre-action discipline integrity); SSOT scope 8 files → 10 files. Harness R33-88..91 (+4 checks); EXPECTED_INDEX_COUNTER 396 → 400. **Prior 2026-05-12 Task 3 — External Knowledge Surface governance §10b new section + 8 cross-cutting integrations:** §10b new (~60 lines) covers external knowledge surface integration — trigger / two access modes / two AI access variants / Cloud-side destructive op safety / Mode switch protocol / tool neutrality. Cross-cutting: §1 conditional read external knowledge surface pointer; §2 new rule 7 External KB in source-of-truth priority; §3 PLAN onboarding readiness check external KB; §3 PERSIST external KB sync during PERSIST; §4 closeout external KB sync check at closeout; §3.6 external KB wizard variant + PROFILE.md schema; §5a backup list. New files: `dev/templates/external_kb_template.md` + `docs/EXTERNAL_KB_COOKBOOK.md`. Support file updates: `dev/wizards/playbook.md` Variant section; `dev/wizards/README.md`; `dev/CODEBASE_CONTEXT.md` template; `dev/DOC_SYNC_CHECKLIST.md` new row. INIT.md POST-INSTALL extended Message 1+2 to Message 1+2+3 (added external KB prompt). Harness R33-70..87 (+18 checks, 9 mirror parity pairs); EXPECTED_INDEX_COUNTER 378 → 396. **Prior 2026-05-12 Task 1 third batch — register signals disambiguation:** §11a rule 5 框架第一層措辭由 `everyday language` 明確化為 `register matching user's chat language AND actual register signals from user's messages`;發送前自我審查由 1 scan 拆 2 scan,Scan 2 新加 `User-AI register mismatch detection` 比對 AI 草稿與用戶最近 3 條訊息的 register signals;正例反例段落由粵語加中英夾雜重寫為 4 種語言並列(英文 / 繁體書面 / 簡體書面 / 日文)。Harness R33-66..69(+4 checks)mirror parity AGENTS↔INIT;EXPECTED_INDEX_COUNTER 374 → 378。**Prior 2026-05-12 Task 1 Language layer separation framework — §11a rule 5 重構:** §11a rule 5 由單 paragraph 重構成三層 framework。Opening explicit default `Assume the user may not be a software developer`,Layer 1 scope cover 6 種 chat surface;commit message / code comment / governance file internal text 屬 Layer 2 不受 Layer 1 紀律約束。Layer 2 governance internal naming 加 explicit file scope enumeration (`AGENTS.md` / `INIT.md` / `dev/*` / `docs/qa/*` / `docs/releases/*` internal-only),容許英文 anchor 內部 reference。Layer 3 governance internal schema files (`dev/SESSION_HANDOFF.md` / `dev/SESSION_LOG.md` / `dev/CODEBASE_CONTEXT.md` / `dev/PROJECT_MASTER_SPEC.md`) 必須英文 schema (cross-AI agent 互操作必要);user-facing surface 跟用戶 locale;harness anchor 跨層豁免。多語言通用聲明 + 3 locale (EN / 中 / ja) example (`applies to any natural language`)。Rule 9 / Rule 10 保留獨立 (wording 不變),framework cross-reference。既有 Banned-as-sentence-subject + Ground truth carve-out + Pre-ship self-check + Violation handling reframe 入 Layer 1 (R33-28..31 既有 anchor key phrase preserve)。Harness R33-58..65 (+8 checks) mirror parity AGENTS↔INIT;EXPECTED_INDEX_COUNTER 366 → 374。**Prior 2026-05-12 Task 2 hardening — Pre-action discipline + File proliferation + Closeout stray-file scan:** §3 READ minimum coverage extended from 4 to 6 items (memory-not-source clause cross-ref §11a rule 4 + new-file three-step verify); §3b adds `File proliferation discipline` sub-section (named *Throwaway / orphan file* anti-pattern + forbidden scratch / temp / `_old` / `_tmp` / dated-session-suffix names + every kept file must have stable purpose recorded); §4 closeout adds `Closeout stray-file scan` mandatory action before smart-skip gate. Harness R33-50..57 (+8 checks) mirror parity AGENTS↔INIT; EXPECTED_INDEX_COUNTER 358 → 366. **Prior 2026-05-09 post-v3.0.10 governance extension — §0b local-tool / SDK / skill alignment principle: §0b 第 2 段尾追加 1 段強調 `Same alignment principle applies to local CLI tools, installed SDKs, package-manager invocations, and project-level skills: prefer documented sources (project SSOT > skill description / runbook > --help / --version self-check > official docs) over training-memory syntax`,reuse 既有 §11a rule 4 `UNVERIFIED` 機制,non-breaking。Harness R33-48/49 (+2) mirror parity AGENTS↔INIT;EXPECTED_INDEX_COUNTER 356 → 358。Prior v3.0.10 release (shipped 2026-05-08)** — worktree edge case governance: §1 `Worktree fallback (mandatory)` for skip-worktree files (`dev/SESSION_HANDOFF.md` / `dev/SESSION_LOG.md` filtered out of worktree checkout); §3c Phase 1 step 2 `Canonical execution locus` sub-bullet (worktree-path harness execution triggers spurious `H01` / `R27-10` failures by-design per skip-worktree convention; main repo path is canonical). Plus combined zh-TW LOW residual sweep (README.zh-TW.md `入面` / `令` / `用戶` tokens at v3.0.8 row converged to formal-written register matching v3.0.9 sweep policy). Harness R33 series extended R33-42..47 (+6 checks); EXPECTED_INDEX_COUNTER 350 → 356. Prior v3.0.9 release (shipped 2026-05-07, commit `f4038d2`, tag `5228701`) + post-tag README i18n register polish (commit `28dc1e8` Round 3 + Round 4 i18n inline-English fragment density reduction)** — v3.0.8: paradigm response patch (POST-INSTALL Profile Selection list reflow + install flow two-message split + wizard Step 1 main+supplements frame + Source-grounding discipline + labeled assumption tags + 4 Chinese-mixed-into-English defects fix + matrix-qc deep audit fix-pass). Reply-tone hard-rule patch (round 1): §11a rule 5 plain-language strengthen (Chinese-context inline-English-mixing ban + explicit governance internal ID list ban + ground truth carve-out for file path / git SHA / version number / commit hash / function / variable / command names); §11a rule 2 outcome-first option framing (each option's label leads with impact / outcome / user's goal, not mechanism); header startup inventory ghost-reference fix (§0a added to line 5 inventory); §5a bootstrap backup list ghost-reference fix (`dev/wizards/` + `dev/templates/` files added — v3.0.7+ governance files were missing); cross-doc baseline drift cleanup (Snapshot status grep-anchor 290 → 325 with R33×35). Work-pool boundary patch (round 2): §1 `Work-pool boundary (mandatory)` added — handoff prompt block is seed context only; its description of prior-session done / changed / pending is HISTORY, not part of this session's work pool; §4 `Session work-pool boundary (mandatory)` added — SESSION_LOG entry's Completed / Changed / Done / Files-this-session-edited fields strictly reflect work actually performed in THIS conversation; cross-session continued tasks record only the increment, not cumulative completion. Cross-reference between §1 and §4 hard-separates handoff seed from this session's record at both endpoints. Root cause addressed: old INIT.md users reported recurring SESSION_LOG drift (prior-session done items copy-pasted into this session record) — prior governance left work-pool boundary to mental tracking, which drifted in practice. Harness R33 series extended (R33-28..37 round 1, R33-38..41 round 2, +14 checks total); EXPECTED_INDEX_COUNTER 336 → 350.
...
- R33 series (41 checks after 2026-05-08 worktree edge case patch — adds R33-42..47): Onboarding Wizard System — `dev/wizards/` directory (R33-01), `dev/wizards/playbook.md` behavior layer (R33-04), `dev/wizards/README.md` (R33-05), `dev/templates/` namespace + `spec_template.md` + `runbook_template.md` (R33-18..20), AGENTS.md + INIT.md FILE 1 mirror §3.6 Onboarding Wizard System parity (R33-10..11), §5a backup list PROFILE.md+RUNBOOK.md presence (R33-12..13), INIT.md install POST-INSTALL: Profile Selection step (R33-14), §3 PLAN reads PROFILE.md + handles wizard_disabled_spec flag (R33-15..16), INIT.md PROFILE.md template includes wizard_disabled_spec field (R33-17). **v3.0.8 paradigm response additions (R33-21..27): Source-grounding discipline parity (R33-21/22), labeled assumption tags `[from your input]` / `[my inference]` (R33-23/24), INIT.md install Setup Completion + Optional Wizard section (R33-25), Message 1 'Governance framework ready' (R33-26), two-separate-messages discipline (R33-27).** **2026-05-07 round-1 reply-tone hard-rule additions (R33-28..37): §11a rule 5 inline-English-mixing ban parity (R33-28/29), §11a rule 5 ground truth carve-out parity (R33-30/31), §11a rule 2 outcome-first option framing parity (R33-32/33), §5a backup list includes dev/wizards/playbook.md parity (R33-34/35), header startup inventory includes §0a parity (R33-36/37).** **2026-05-07 round-2 work-pool boundary additions (R33-38..41): §1 Work-pool boundary parity AGENTS↔INIT (R33-38/39), §4 Session work-pool boundary parity AGENTS↔INIT (R33-40/41).** **2026-05-08 worktree edge case additions (R33-42..47): §1 Worktree fallback parity AGENTS↔INIT (R33-42/43), §3c Canonical execution locus parity AGENTS↔INIT (R33-44/45), skip-worktree convention reference parity AGENTS↔INIT (R33-46/47).** Retired in v3.0.7: R33-02/03 (deleted file existence checks), R33-06/07 (i18n parity for deleted schema files), R33-08/09 (no-hardcoding blacklist for deleted schema files).
- Pre-existing checks adjusted: EXPECTED_INDEX_COUNTER 411→417 (Root-fix Option 4 +6 R33-103..108); prior 407→411 (Combined Option C + Closeout fix +4 R33-99..102); LATEST_STABLE_TAG v3.0.10 (unchanged — post-tag polish only)
- Root-fix Option 4 (R33-103..108, 2026-05-12): mirror parity AGENTS↔INIT — §4 Stray-file Disposition mandatory visible output (R33-103/104, `Closeout stray-file disposition (mandatory visible output)` phrase); §4 Compactness Budget Check mandatory visible output (R33-105/106, `Closeout compactness budget check (mandatory visible output)` phrase); §4 Same-session rule self-audit + Author-then-Violate anti-pattern (R33-107/108, `Same-session rule self-audit (mandatory)` phrase).
- Combined Option C + Closeout fix (R33-99..102, 2026-05-12): R33-99/100 §3 PERSIST user-facing artefact audit conditional sub-rule mirror parity AGENTS↔INIT (`user-facing artefact audit` phrase); R33-101 §4 closeout intent-detection multilingual trigger combined AGENTS + INIT mirror count (`detect by intent, not strict keyword match` phrase, ≥2 total); R33-102 §4 closeout triple-backtick markdown code block explicit AGENTS + INIT mirror count (`triple-backtick markdown code block` phrase, ≥2 total).
- Recommended 3 design-layer fixes (R33-94..98, 2026-05-12 Path A): §5a step 5 Install mode line mirror parity AGENTS↔INIT (R33-94/95, `Install mode:` phrase); INIT.md FILE 1 INSTALL_MERGE_OK gate (R33-96); INIT.md FILE 2/3 first-10-lines dup guard (R33-97, `first 10 lines of the existing file` phrase); INIT.md FILE 1 `Section-Aware Merge Protocol` header (R33-98).
- INIT install flow dry-run fix (R33-92/93, 2026-05-12): R33-92 grand statement "Install flow is a continuous pipeline" present in INIT.md head (≥1 hit); R33-93 Message 1→2 back-to-back transition "back-to-back without waiting for user reply" present (≥1 hit).
- Landing page audit + Matrix-QC upgrade (R33-88..91, 2026-05-12): landing page f13 / f14 feature card present checks (R33-88/89, `data-i18n="f13_t"` / `data-i18n="f14_t"`); landing page f13_t / sc4_t 4-locale parity (R33-90/91, `f13_t:` count = 4 / `sc4_t:` count = 4 across tw / en / cn / ja locale blocks). Matrix-QC upgrade is gitignored internal tool change, not harness-tracked.
- Task 3 External Knowledge Surface governance §10b (R33-70..87, 2026-05-12): mirror parity AGENTS↔INIT — §10b heading (R33-70/71, `External Knowledge Surface` phrase); Mirror mode + Bridge mode (R33-72/73, `Mirror mode` phrase); Cloud-side destructive op safety (R33-74/75); Mode switch protocol (R33-76/77); §1 conditional read external knowledge surface pointer (R33-78/79); §2 External KB in source-of-truth priority (R33-80/81); §3 PERSIST external KB sync during PERSIST (R33-82/83); §4 closeout external KB sync check at closeout (R33-84/85); §3.6 external KB wizard variant (R33-86/87). Reframe-preserve verified: R33-28..69 既有 anchor key phrase 仍 hit。
- Task 1 third batch — register signals disambiguation (R33-66..69, 2026-05-12): mirror parity AGENTS↔INIT — register signals clause (R33-66/67, `register signals from user's messages` phrase); User-AI register mismatch detection (R33-68/69, `User-AI register mismatch detection` phrase). Reframe-preserve verified: R33-58..65 既有 8 條 Task 1 second batch anchor 仍 hit;R33-28..31 既有 anchor key phrase 仍 hit。
- Task 1 Language layer separation framework (R33-58..65, 2026-05-12 second batch): mirror parity AGENTS↔INIT — non-developer default (R33-58/59, `Assume the user may not be a software developer` sentence); `Language layer separation` framework name (R33-60/61); Layer 3 `Governance internal schema files` carve-out (R33-62/63); multilingual universality `applies to any natural language` (R33-64/65).
- Task 2 hardening (R33-50..57, 2026-05-12): mirror parity AGENTS↔INIT — §3 READ memory-not-source clause (R33-50/51, `files not Read in this session are treated as` sentence); §3 READ new-file three-step verify (R33-52/53, `Before creating a new file, verify no existing file` sentence); §3b `File proliferation discipline` sub-section (R33-54/55); §4 `Closeout stray-file scan` mandatory action (R33-56/57)
- §0b post-v3.0.10 extension (R33-48/49): mirror parity AGENTS↔INIT for `Same alignment principle applies to local` sentence — local CLI / SDK / package-manager / skill alignment guidance reusing §11a rule 4 `UNVERIFIED` machinery
